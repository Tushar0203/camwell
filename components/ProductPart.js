...
import "@/app/styles/productPart.css"
          ...
          rest.forEach((i, index) => {
            document.querySelector(getCard(i)).addEventListener('click', () => {
              if (!clicks) {
                ...
                const clickedPosition = order.indexOf(i);
                // We need to move forward from current position (0) to the clicked position
                // Since order[0] is the active card, we need clickedPosition steps
                clicks = clickedPosition;
                direction = 1;
                step();
              }
            });
            
            // Also add click events to slide numbers
            document.querySelector(getSliderItem(i)).addEventListener('click', () => {
              if (!clicks) {
                // Calculate the position of the clicked card in the current order
                const clickedPosition = order.indexOf(i);
                // We need to move forward from current position (0) to the clicked position
